...
PASTA_SAIDA_MODELOS = os.path.join(FASE2_DIR, 'modelos_fase2_final')
ARQUIVO_LOG = os.path.join(FASE2_DIR, 'RESULTADOS_FASE2_FINAL.csv')

DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
NUM_WORKERS = 0 # Rich as vezes briga com multiprocessamento no Windows, 0 é mais seguro para UI
SEED = 42
...
def treinar_ciclo(config):
    ...
    # Determina resolução para display
    ...
    console.print(Panel.fit(
        ...
        f"[bold white]Batch:[/][cyan] {batch_size}[/] | [bold white]RadImageNet:[/][magenta] {'Sim' if usar_rad and 'resnet' in model_name else 'Não'}[/]\n"
        f"[bold white]Resolução:[/][green] {display_size}px[/]",
        title="🚀 Iniciando Experimento",
        border_style="blue"
    ))

    # --- SETUP (Com Spinner) ---
    with console.status("[bold green]Preparando dados e modelo...", spinner="dots"):
        # Ajusta resolução baseado no modelo
        if 'beit' in model_name:
            current_size = 224
        elif 'swin' in model_name and '256' in model_name:
            current_size = 256
    ...
    # --- TRAINING LOOP (Com Tabela Live e Barra de Progresso) ---
    best_loss = float('inf')
    patience_count = 0
    save_path = os.path.join(PASTA_SAIDA_MODELOS, f"{model_name}_fase2.pth")
    ...
    with Live(Panel(dashboard_table, title="Monitoramento em Tempo Real", border_style="green"), refresh_per_second=4) as live:
        ...
        for epoch in range(EPOCHS):
            ...
            # Adiciona tarefa na barra de progresso
            task_id = progress.add_task(f"[cyan]Epoca {epoch+1}/{EPOCHS}", total=len(train_loader))
            
            # Precisamos iterar manualmente para atualizar o Live corretamente com a barra externa
            # (Rich Live e Progress juntos exigem um pouco de malabarismo, então simplificamos usando o update)
            ...
            for i, (imgs, lbls) in enumerate(train_loader):
                ...
                # Atualiza título do painel apenas para dar feedback visual de movimento
                if i % 5 == 0:
            ...
            progress.remove_task(task_id) # Limpa barra da época anterior
...
def main():
    console.rule("[bold blue]SISTEMA DE TREINAMENTO DE TCC v2.0[/]")

    # Debug: Mostrar caminhos configurados
    console.print(f"[dim]PASTA_BASE: {PASTA_BASE}[/]")
    console.print(f"[dim]CSV_CONTROLE: {CSV_CONTROLE}[/]")
    console.print(f"[dim]CSV existe? {os.path.exists(CSV_CONTROLE)}[/]")
    console.print()
